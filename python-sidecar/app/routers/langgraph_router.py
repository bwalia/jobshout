...
from app.runners import langgraph_runner
...
async def run_langgraph(req: RunRequest, _: None = Depends(_verify_secret)):
    """Execute a LangGraph workflow."""
    logger.info(
        "LangGraph execution request",
        extra={"execution_id": req.execution_id, "agent_id": req.agent_id},
    )
    return langgraph_runner.run(req)
