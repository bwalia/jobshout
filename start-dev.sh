...
(
  ...
  GMAIL_TOKEN_KEY="${GMAIL_TOKEN_KEY:-}" \
  GMAIL_OAUTH_REDIRECT_URL="${GMAIL_OAUTH_REDIRECT_URL:-http://localhost:8080/api/v1/mail/connection/oauth/callback}" \
  MAIL_POLL_INTERVAL="${MAIL_POLL_INTERVAL:-5m}" \
  MAIL_RECONCILE_INTERVAL="${MAIL_RECONCILE_INTERVAL:-15s}" \
  ...
  WSLPROXY_ENABLED="${WSLPROXY_ENABLED:-true}" \
  WSLPROXY_BASE_URL="${WSLPROXY_BASE_URL:-}" \
  WSLPROXY_API_TOKEN="${WSLPROXY_API_TOKEN:-}" \
  WSLPROXY_USERNAME="${WSLPROXY_USERNAME:-}" \
  WSLPROXY_PASSWORD="${WSLPROXY_PASSWORD:-}" \
  WSLPROXY_PLATFORM="${WSLPROXY_PLATFORM:-openresty-admin-next}" \
  WSLPROXY_PROFILE="${WSLPROXY_PROFILE:-prod}" \
  WSLPROXY_TIMEOUT="${WSLPROXY_TIMEOUT:-30s}" \
  WSLPROXY_LAB_MAX_RUNTIME="${WSLPROXY_LAB_MAX_RUNTIME:-10m}" \
  WSLPROXY_TARGET_ALLOWLIST="${WSLPROXY_TARGET_ALLOWLIST:-}" \
  ...
  STRIX_BASE_URL="${STRIX_BASE_URL:-http://localhost:11436}" \
  STRIX_JWT_SECRET="${STRIX_JWT_SECRET:-}" \
  STRIX_TIMEOUT="${STRIX_TIMEOUT:-60s}" \
  STRIX_POLL_INTERVAL="${STRIX_POLL_INTERVAL:-15s}" \
  STRIX_MAX_RUNTIME="${STRIX_MAX_RUNTIME:-2h}" \
  STRIX_TARGET_ALLOWLIST="${STRIX_TARGET_ALLOWLIST:-}" \
