...
func (r *Runner) writeOne(
	...
	plan, err := r.plan(ctx, r.structuredModel(req.Model), brief, rb)
	if err != nil {
		return nil, err
	}
	...
	markdown, err := r.draft(ctx, r.proseModel(req.Model), brief, rb, plan)
	if err != nil {
		return nil, err
	}
	...
	c, err := r.review(ctx, r.structuredModel(req.Model), rb, plan, markdown)
	switch {
	case err != nil:
	...
	default:
		...
		revised, rerr := r.revise(ctx, r.proseModel(req.Model), rb, plan, markdown, c)
		if rerr != nil {
	...
	if words := wordCount(markdown); words < MinArticleWords {
		...
		expanded, eerr := r.expand(ctx, r.proseModel(req.Model), brief, rb, plan, markdown, words)
		switch {
		case eerr != nil:
